...
void addChild(char *nameP, char* nameC){
    ...
    if(childp = NULL)
        p->leftMostChild = makeNode(nameC);
...
void  printTree2(Node* root){
    if(root == NULL) return ;
    printf("%s \t", root->name);
    printTree2(root->leftMostChild);
    printTree2(root->rightSibling);
}
...
void load(FILE *f){
    ...
    while(!feof(f)){
        fscanf(f,"%s",name);
        if(strcmp(name,"$$") == 0) {
            printf("Traversal all file !!!!\n");
            break;
        }

        //reassign index equal 0( next row);
            ...
            if(index == 1){
                p = find(root,name);
                if(p == NULL) p = makeNode(name);
                //make note that index 1 of row 1 is root of all tree.
                if(toggle == 0) {
                    root = p;
                    toggle = 1;
                }
                ...
                p->leftMostChild = find(root,name);
                if(p->leftMostChild == NULL) 
                    p->leftMostChild = makeNode(name);
                p = p->leftMostChild;
            }
            else{
                Node *q = p;
                ...
                q->rightSibling = find(root,name);
                if(q->rightSibling == NULL) q->rightSibling = makeNode(name);
            }
        }
    }
}
...
int main(){
    while (1){
        ...
        else if (strcmp(cmd,"Print")==0) printTree2(root);
